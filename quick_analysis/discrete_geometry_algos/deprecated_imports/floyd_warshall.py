...
class FloydWarshall(object):
    ...
    .. math::
        ...
        \end{matrix}y
    """
    __slots__ = ['edge_index', 'is_undirected', 'weights', 'true_weights_distance', 'debug']
    INF = float('inf')
    ...
    def __init__(self,
        ...
        @param true_weights_distance: if True, returns uses actual weights as distance factors, instead of their normalised version
        ...
        self.debug = debug
        self.weights = torch.ones(len(edge_index))/len(edge_index) if weights is None else\
              (weights/weights.sum() if true_weights_distance is False else weights)
        _debug_tensor('FloydWarshall.weights', self.weights, self.debug)
    ...
    def __call__(self) -> torch.Tensor:
        ...
        _debug_tensor('FloydWarshall.shortest_distances', shortest_distances, self.debug)
    ...
    def __init_shortest_distances(self) -> torch.Tensor:
        ...
        _debug_tensor('FloydWarshall.initial_distances', distances, self.debug)
        return distances
